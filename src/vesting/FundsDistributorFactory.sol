...
contract FundsDistributorFactory is Ownable {
        ...
        Ownable(msg.sender)
    {
        ...
        require(
            _start + _cliffDuration > block.timestamp,
            "FundsDistributorFactory::constructor: cliff time is before current time"
        );
    ...
    function deployFundee(
        ...
        returns (bool)
    {
        FundsDistributor fundeeContract =
            new FundsDistributor(beneficiary, start, cliffDuration, duration, revocable, _identifier);
     ...
     */
    function withdrawTokens(uint256 amount) external onlyOwner returns (bool) {
